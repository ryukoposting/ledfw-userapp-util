...
pub trait OutputFormat {
    ...
    fn write_all_cmds<'s>(&mut self, cmds: Vec<BleCommand<'s>>) -> Result<(), Self::Error> {
        for cmd in cmds {
            self.write_cmd(cmd)?;
        }
        Ok(())
...
impl ElfToBle {
    ...
    pub fn generate<'s, O: OutputFormat>(&self, file: &'s File, mut out: O, do_commit: bool) -> Result<O::Finish, O::Error> {
        let mut result = vec![BleCommand::Start];
        ...
        if do_commit {
            result.push(BleCommand::Commit { crc });
        } else {
            result.push(BleCommand::Run { crc });
        }
